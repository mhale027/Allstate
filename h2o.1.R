...
test <- cbind(alldata[te,], pred.test.dnn.1)
...
test <- cbind(alldata[te,], pred.test.dnn.1, pred.test.dnn.2)
...
pred <- (pred.test.dnn.1 + pred.test.dnn.2 + pred.test.dnn.3)*mean.loss/3
sample$loss <- pred
...
write.csv(sample,'submission.11.1.3.csv',row.names = FALSE)
